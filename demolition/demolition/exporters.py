...
class SeaTimesSnippetsExporter(exporters.BaseItemExporter):
	"""
	get snippet images from Seattle Times on NewsBank.
	"""

	def export_item(item):
		"""
		OCR them to see if they actually contain the address text
		store OCRed URLs to Redis so we don't try them again
		save out matching images to filesystem
		"""
...
class SeaTimesTextToImageExporter(exporters.BaseItemExporter):
	"""
	process item 'text' fields
	"""
